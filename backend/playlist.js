...
playlist_api_route.get("/user_follow/:UserID", function(req, res){
    ...
    connection.query("SELECT p.PlaylistID, PlaylistName, PlaylistIMG, p.TimeCreated, PlaylistCreator, u.UserName FROM UserFollowPlaylist ufp INNER JOIN Playlist p on ufp.PlaylistID = p.PlaylistID INNER JOIN User u ON p.PlaylistCreator = u.UserID WHERE ufp.UserID = ? AND p.IsDeleted = False;", user_id, function(error, results, fields){
